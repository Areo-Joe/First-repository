    ...
    opts = d3._extend(
      {
        ...
        prepareData: function(graph) {
          ...
          for (let c = 0; c < links_length; c++) {
            ...
            if (typeof(links[c].source) === 'object') {
              sourceIndex = nodesMap[links[c].source.id];
              targetIndex = nodesMap[links[c].target.id];
            } else {
              sourceIndex = nodesMap[links[c].source];
              targetIndex = nodesMap[links[c].target];
            }
            
            // console.log(typeof(links[c].source))
            // ensure source and target exist
            // console.log(c)
            if (!nodes[sourceIndex]) {
              // console.log(links[c].source)
              // // console.log(c)
